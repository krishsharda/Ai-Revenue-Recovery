...
// `/settings/email/test` is the only guarded endpoint (the destructive admin
// routes were removed entirely). The operator pastes the server's ADMIN_TOKEN
// in Settings; it is held for the browser session only and is never bundled
// into the app, so no secret ships to the client. Server-side rendering never
// calls it.
//
// `/api/settings` still reports `email.test_allowed`, which is what lets the UI
// explain *why* a send is unavailable when no token has been entered, instead
